...
CMD ["python", "-m", "gunicorn", "main:app", "--workers", "2", "--worker-class", "uvicorn.workers.UvicornWorker", "--bind", "0.0.0.0:8000", "--timeout", "120", "--keepalive", "5", "--access-logfile", "-", "--error-logfile", "-", "--log-level", "info"]
