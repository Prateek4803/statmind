"""
StatMind — API v6: Sessions 1-6 with PDF Report Generation
Run: uvicorn api6:app --port 8010
"""
import os, uuid, tempfile, dataclasses, json
import numpy as np
...
app = FastAPI(title="StatMind API", version="6.0.0")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

# Temp storage for generated PDFs (keyed by report_id)
_report_cache = {}
...
@app.get("/health")
def health(): return {"status":"ok","service":"StatMind","version":"6.0.0"}

# ── Sessions 1-5 (same as v5) ─────────────────────────────────────────────────
...
# ── Session 6: PDF Report Generation ─────────────────────────────────────────
@app.post("/api/v1/report/generate")
async def generate_pdf_report(request: Request):
    """
    Body: {
      normality_result, capability_result, spc_result, grr_result, capa_result,
      meta: { parameter, process, n }
    }
    Returns: { report_id, download_url, pages_estimate }
    """
    body = await request.json()
    ...
    try:
        ...
        sections = sum([1 for k in ['normality_result','capability_result','spc_result','grr_result','capa_result']
                        if body.get(k)])
        return jd({
            "success": True,
            "report_id": report_id,
...
async def download_report(report_id: str):
    """Download a previously generated PDF by report_id."""
