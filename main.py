...
main_patch.py — StatMind Missing API Endpoints
================================================
INSTRUCTIONS: Append the content below to the END of main.py,
immediately before the  `if __name__ == "__main__":`  block.

These 11 endpoints are referenced by the frontend but were never
added to the live main.py.  Symptoms in production:
  - /api/v1/capability/ci   → 404  (Cpk confidence intervals)
  - /api/v1/outliers/analyze → 404  (outlier detection)
  - /api/v1/equivalence/analyze → 404  (TOST equivalence)
  - /api/v1/runchart/analyze → 404  (run chart)
  - /api/v1/cusum/analyze    → 404  (CUSUM)
  - /api/v1/ewma/analyze     → 404  (EWMA)
  - /api/v1/correlation/matrix → 404 (correlation heatmap)
  - /api/v1/sample-size/calculate → 404 (sample size)
  - /api/v1/hypothesis/two-way-anova → 404
  - /api/v1/rsm/design        → 404
  - /api/v1/rsm/analyze       → 404
